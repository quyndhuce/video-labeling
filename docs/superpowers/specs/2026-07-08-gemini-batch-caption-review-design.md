...
5. **Segment-level captions are now in scope too.** The "Scope" section below originally excluded segment-level captions entirely because they have no `visual_caption` field. Real project data showed a segment carries its own `contextual_caption`/`contextual_caption_vi` (e.g. describing "Hang Dau Water Tower" as a whole) separate from its objects' `visual_caption`s, and that field has the same proper-name-vs-generic-type problem. `_iter_review_candidates` now yields one row per segment (its `contextual_caption`) in addition to one row per object region (its `visual_caption`), tagged with a `level: 'segment' | 'object'` field. `_CAPTION_FIELDS` maps level to the (english, vietnamese) field pair to read/write; `_run_batch_review` and `/batch-review/confirm` pick the right field pair per item via each result's `caption_field`. Only `contextual_caption` is touched at the segment level — `knowledge_caption` and `combined_caption` remain out of scope. As before, only the English field is ever sent to Gemini; the Vietnamese field is always the model's own translation of the corrected English text, never independently "fixed".
